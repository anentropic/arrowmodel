...
This document provides guidance for developers working on arrowmodel.
...
# Test coverage
uv run pytest --cov=src/arrowmodel --cov-report=term
```
...
```
arrowmodel/
├── src/arrowmodel/      # Package source code
...
# Place in src/arrowmodel/py.typed (marker only)
# or src/arrowmodel/*.pyi (stub files)
```
